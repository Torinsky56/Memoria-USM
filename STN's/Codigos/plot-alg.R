...
std_ncol <-  "gray" # Local Optima Gray 
end_ncol <-  "gold"  # End of trajectories for each run.
start_ncol <-  "black"   # Start of trajectories
...
stn_decorate <- function(N, bmin)  {
  ...
  V(N)[V(N)$Type == "best"]$size = V(N)[V(N)$Type == "best"]$size + 0. # Increase a bit size of best node
...
print("Number of nodes in STNs ploted:")
